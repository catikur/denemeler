...
class Strategy:
    ...
    def _open_position(self, signal_result: int):
        ...
        if self.stop_loss < price < self.take_profit:
    ...
    def _check_tp_sl(self, trade: Trade):
        ...
        if trade.side == "long":
            if self.stop_loss is not None:
                if price + (trade.entry_price - self.stop_loss) <= self.stop_loss:
                    sl_triggered = True
            if self.take_profit is not None:
                if price >= self.take_profit + (self.take_profit - self.stop_loss):
                    tp_triggered = True

        elif trade.side == "short":
            if self.stop_loss is not None:
                if price <= self.stop_loss - (self.take_profit - self.stop_loss):
                    tp_triggered = True
            if self.take_profit is not None:
                if self.take_profit + (self.take_profit - trade.entry_price) <= price:
                    sl_triggered = True
...
class BreakoutStrategy(Strategy):
    ...
    def _check_signal(self) -> int:
        ...
        if self.candles[-1].close > self.candles[-2].high and self.candles[-2].close > self.candles[-3].close and\
                self.candles[-1].volume > \
                (self.candles[-2].volume + self.candles[-3].volume + self.candles[-4].volume) / 3:
            return 1
        elif self.candles[-1].close < self.candles[-2].low and self.candles[-2].close < self.candles[-3].close and\
                self.candles[-1].volume > \
                (self.candles[-2].volume + self.candles[-3].volume + self.candles[-4].volume) / 3:
            return -1
        else:
